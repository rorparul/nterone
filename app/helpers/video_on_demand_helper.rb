module VideoOnDemandHelper
  ...
  def video_completed_class(video_module, user)
    ...
    video_module.watched_count(user) == video_module.videos.count ? 'text-success' : ''
  end
  ...
  def exam_completed_class(video_module, user)
    ...
    video_module.completed_exams_count_for(current_user) == video_module.exams_count ? 'text-success' : ''
  end
end
