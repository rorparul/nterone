class Reports::SalesController < ApplicationController
  ...
  def create
    ...
    @company_sales = Opportunity.where(date_closed: start_date..end_date, employee_id: nil, reason_for_loss: nil).where.not(customer_id: nil)
    @opportunities = Opportunity.where(date_closed: start_date..end_date, reason_for_loss: nil).where.not(employee_id: nil, customer_id: nil)
